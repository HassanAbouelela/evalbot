...
class Snekbox(Cog):
    ...
    async def upload_output(self, output: str) -> Optional[str]:
        ...
        return await hastebin.send_to_paste_service(self.bot, output, extension="txt")
    ...
    def prepare_input(code: str) -> str:
        ...
        log.debug(f"Extracted {info} for evaluation:\n{code}")
        return code
    ...
    async def send_eval(self, ctx: Context, code: str) -> Message:
        ...
        async with ctx.typing():
            ...
            self.bot.loop.create_task(wait_for_deletion(self.bot, response, (ctx.author.id,)))
    ...
    async def get_code(self, message: Message) -> Optional[str]:
        ...
        """
        log.debug(f"Getting context for message {message.id}.")
        new_ctx = await self.bot.get_context(message)

        if new_ctx.command is self.eval_command:
            log.debug(f"Message {message.id} invokes eval command.")
            split = message.content.split(maxsplit=1)
            code = split[1] if len(split) > 1 else None
        else:
            log.debug(f"Message {message.id} does not invoke eval command.")
    ...
    async def eval_command(self, ctx: Context, *, code: str = None) -> None:
        ...
        Code can be re-evaluated by editing the original message within 10 seconds
        and clicking the reaction that subsequently appears.
...
def reaction_check(
    ...
    *,
    ...
    is_moderator = (
        allow_mods
        and any(role.id in (constants.admin_role, constants.mod_role) for role in getattr(user, "roles", []))
    )
    ...
    if user.id in allowed_users or is_moderator:
        ...
        log.debug(f"Ignoring reaction {reaction} by {user} on {reaction.message.id}: disallowed user.")
        return False
...
async def wait_for_deletion(
    ...
    deletion_emojis: Sequence[str] = (constants.TRASHCAN,),
    timeout: float = 60 * 5,
    ...
    if attach_emojis:
        for emoji in deletion_emojis:
            try:
                ...
                log.debug(f"Aborting wait_for_deletion: message {message.id} deleted prematurely.")
                return
